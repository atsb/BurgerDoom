...
Word P_Ticker(void)
{
	...
	/* Collect modern mouse input once at the 30 Hz live-gameplay boundary.
	 * This path is separate from the 3DO Pad* stream and is explicitly
	 * discarded during demo recording/playback. */
	D3DO_BeginTicInput();
...
/* Run player actions */
	...
	/* Apply modern mouse rotation only to live gameplay after the original
	 * 3DO Pad* player step. It never reaches the demo command stream. */
	if (!DemoPlayback && !DemoRecording &&
		!(players.AutomapFlags & AF_OPTIONSACTIVE) &&
		pl->playerstate != PST_DEAD && pl->mo) {
		pl->mo->angle += D3DO_GetMouseTurnForTic();
	} else {
		(void)D3DO_GetMouseTurnForTic();
	}
...
void P_Drawer(void)
{
	if (gamepaused) {
		if (refreshdrawn) {
			D3DO_ShowPausePlaque();	/* Overlay pause plaque on the current frame. */
			refreshdrawn = FALSE;
		}
		return;
