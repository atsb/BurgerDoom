...
#define FINALEBUFFERSIZE 4096

static Byte CustomFinaleString[FINALEBUFFERSIZE];
static Boolean CustomFinaleStringLoaded = FALSE;

static void F_LoadCustomFinale(void)
{
	FILE *File;
	Word Size;
	int c;

	if (CustomFinaleStringLoaded) {
		return;
	}
	CustomFinaleStringLoaded = TRUE;

	File = fopen("FINALE.TXT", "rb");
	if (!File) {
		return;
	}
	...
	while ((c = fgetc(File)) != EOF) {
		...
		if (Size >= FINALEBUFFERSIZE - 1) {
			fclose(File);
			CustomFinaleString[0] = 0;
			return;
		}
		CustomFinaleString[Size++] = (Byte)c;
	}
	fclose(File);
...
void F_Drawer(void)
{
	...
	if (status==fin_endtext) {
		...
		FinaleString = EndTextString;
		if (CustomFinaleStringLoaded && CustomFinaleString[0]) {
			FinaleString = CustomFinaleString;
		}
		Temp = FinaleString[TextIndex];	/* Get the final char */
		FinaleString[TextIndex] = 0;		/* End the string here */
		F_PrintString(STARTX,STARTY,FinaleString);	/* Print the string */
		FinaleString[TextIndex] = Temp;	/* Restore the string */
