...
typedef struct state_s {  		/* An actor's state */
	LongWord SpriteFrame;	/* Which sprite resource (upper 16) and frame (lower 8) to display. */
...
typedef struct {		/* Describe an actor's basic variables */
	...
	LongWord flags;			/* Generic state flags */
...
/* interaction info */
	...
	LongWord flags;		/* State flags for object */
...
extern Word ReadDemoWord(const Word *p);
extern void WriteDemoWord(Word *p, Word value);
...
extern const char *HostRecordDemoPath;
extern const char *HostPlayDemoPath;
...
extern Fixed LineOpening(line_t *linedef);
...
extern int G_RecordDemoToFile(const char *filename);
extern int G_PlayDemoFile(const char *filename);
...
extern void D3DO_BeginTicInput(void);
extern angle_t D3DO_GetMouseTurnForTic(void);
...
extern void D3DO_ShowLoadingPlaque(void);
extern LongWord ReadTick(void);
extern void DrawPlaque(Word RezNum);
extern void D3DO_ShowPausePlaque(void);
extern int D3DO_GetKeyboardWeapon(void);
extern Boolean D3DO_DisplayRefreshNeeded(void);
extern void D3DO_RefreshDisplay(void);
