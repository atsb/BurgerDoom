...
static void ClipPixel(Word x,Word y,Word Color)
{
	int sx = (int)x;
	int sy = (int)y;
	if (sx >= 0 && sx < 320 && sy >= 0 && sy < 160) {
		SetAPixel((Word)sx,(Word)sy,Color);
	}
}
...
static void DrawLine(Word x1,Word y1,Word x2,Word y2,Word color)
{
	...
	/* Rebecca's automap deliberately treats these Word coordinates as signed
	 * 16-bit screen offsets.  Make that rule explicit on the SDL host. */
	ax1 += 160;
	ax2 += 160;
	ay1 = 80 - ay1;
	ay2 = 80 - ay2;
	...
	for (;;) {
		ClipPixel((Word)ax1,(Word)ay1,color);
...
extern void D3DO_SetAutomapDirect(int enabled);

void AM_Drawer(void)
{
	D3DO_SetAutomapDirect(1);
