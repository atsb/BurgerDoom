...
/*
 * Host-side USE ray test.
 *
 * The original 3DO path relies on the map blockmap and its integer/fixed-point
 * InterceptVector().  Our REZ/WAD loader can legally reconstruct line bboxes
 * differently from the original 3DO block data, so a USE action must not fail
 * merely because a cached bbox/block index is stale.  This test operates on
 * the actual linedef endpoints and the actual USE ray, while keeping the
 * original LineOpening()/P_UseSpecialLine() semantics after a hit is found.
 */
static Boolean PIT_UseLinesExact(line_t *li)
{
	...
	rx = (double)useline.dx;
	ry = (double)useline.dy;
	sx = (double)(li->v2.x - li->v1.x);
	sy = (double)(li->v2.y - li->v1.y);
	den = rx * sy - ry * sx;
	if (den == 0.0) {
		return TRUE;
	}

	qpx = (double)(li->v1.x - useline.x);
	qpy = (double)(li->v1.y - useline.y);
	tnum = qpx * sy - qpy * sx;
	unum = qpx * ry - qpy * rx;
	...
	/* The player can only use a line intersected by the finite ray and the
	 * finite linedef.  The original code's frac test covered the ray side;
	 * this additionally makes the linedef-side test explicit. */
	if (t < 0.0 || t > 1.0 || u < 0.0 || u > 1.0) {
		return TRUE;
	}

	frac = (Fixed)(t * (double)FRACUNIT + 0.5);
	if (frac < 0 || frac > closedist) {
		return TRUE;
	}
...
void P_UseLines (player_t *player)
{
	...
	/* BurgerDoom/modern host equivalent: the blockmap iterator takes unsigned
	 * Word coordinates, so clamp negative map-block indices before passing them
	 * in. The original C assumes the 3DO blockmap origin keeps these positive,
	 * which is not guaranteed by the host map layout. */
	if (xl < 0) xl = 0;
	if (yl < 0) yl = 0;
...
/*
 * The original 3DO blockmap is normally sufficient, but host-loaded maps
 * can have a blockmap whose origin/indexing does not match the runtime
 * coordinate space. USE is a one-shot action, so fall back to a complete
 * line scan when the block iterator found nothing. This keeps the actual
 * USE semantics in P_UseSpecialLine() and avoids making door interaction
 * dependent on a malformed/translated blockmap.
 */
	if (!closeline) {
		line_t *scan = lines;
		Word remaining = numlines;
		while (remaining--) {
			PIT_UseLinesExact(scan);
			++scan;
		}
	}
