...
#define MAPNAMECOUNT 24
#define MAPNAMELENGTH 128

static Byte CustomMapNames[MAPNAMECOUNT][MAPNAMELENGTH];
static Boolean CustomMapNamesLoaded = FALSE;

static void IN_LoadCustomMapNames(void)
{
	FILE *File;
	char Line[MAPNAMELENGTH + 1];
	Word i;
	Word Length;

	if (CustomMapNamesLoaded) {
		return;
	}
	CustomMapNamesLoaded = TRUE;

	File = fopen("MAPNAMES.TXT", "rb");
	if (!File) {
		return;
	}
	...
	for (i = 0; i < MAPNAMECOUNT; ++i) {
		...
		Line[strcspn(Line, "\r\n")] = 0;
		Length = (Word)strlen(Line);
		if (!Length || Length >= MAPNAMELENGTH) {
			fclose(File);
			return;
		}
		strcpy((char *)CustomMapNames[i], Line);
	}

	fclose(File);
	for (i = 0; i < MAPNAMECOUNT; ++i) {
		mapnames[i] = CustomMapNames[i];
	}
}
