...
/* Decide which patch to use for sprite relative to player */
	...
	PatchData = LoadAResource(lump);	/* Get the sprite group */
	if (!PatchData) {
		return;
	}
	patch = (patch_t *)PatchData;
	Offset = ReadBE32((const Byte *)patch + ((LongWord)(StatePtr->SpriteFrame & FF_FRAMEMASK) * 4u));
	PatchSize = GetAResourceSize(lump);
	if ((Offset & 0x3FFFFFFFUL) >= PatchSize) {
	...
	if (Offset&PT_NOROTATE) {		/* Do I rotate? */
		...
		Offset = ReadBE32((const Byte *)patch + ((LongWord)rot * 4u));		/* Use the rotated offset */
	}
...
/* I also will clip to screen coords */
	...
	/* patch_t offsets are big-endian serialized 16-bit fields. */
	{
		const int32_t leftoffset = (int16_t)ReadBE16((const Byte *)patch);
		Trx -= leftoffset << FRACBITS;
	}
...
/* The shape is sideways, so I get the HEIGHT instead of the width! */
	...
	/* GetShapeHeight returns CEL pixels, while IMFixMul operates on 16.16 Fixed.
	 * Promote the integer sprite width to Fixed before multiplying by the
	 * projected scale.  Without this conversion x2 collapses to x1, leaving
	 * only a one-column sliver of every world sprite. */
	x2 = (IMFixMul((Fixed)GetShapeHeight(&patch->Data)<<FRACBITS,Trz)>>FRACBITS)+x1;
	if (x2 <= 0) {
		ReleaseAResource(lump);
...
/* get light level */
	...
	vis->PatchOffset = (Byte *)patch - (Byte *)PatchData;	/* Shape offset */
	...
	{
		const int32_t topoffset = (int16_t)ReadBE16((const Byte *)patch + 2u);
		Trz = Trz+(topoffset << FRACBITS);
	}		/* Height offset */
...
{
	Word Side;
	if ((uintptr_t)bsp & (uintptr_t)1u) {		/* Is this a BSP sector node? */
