...
static void DrawAWeapon(pspdef_t *psp,Word Shadow)
{
	...
	Byte *Input;
	Byte *Shape;
	LongWord ResourceSize;
	LongWord ShapeOffset;
	int x,y;
	int16_t shapeX, shapeY;
	state_t *StatePtr;

	StatePtr = psp->StatePtr;
	if (!StatePtr)
		return;

	RezNum = StatePtr->SpriteFrame >> FF_SPRITESHIFT;
	ResourceBase = (Byte *)LoadAResource(RezNum);
	if (!ResourceBase)
		return;

	ResourceSize = GetAResourceSize(RezNum);
	Input = (Byte *)GetShapeIndexPtr(
		ResourceBase, StatePtr->SpriteFrame & FF_FRAMEMASK);
	if (!Input || Input < ResourceBase ||
		(uintptr_t)Input - (uintptr_t)ResourceBase >= ResourceSize) {
		ReleaseAResource(RezNum);
		return;
	}

	/*
	 * Weapon image records begin with two signed big-endian 16-bit screen
	 * offsets, followed by the 3DO CEL/CCB data.
	 */
	shapeX = (int16_t)(((uint16_t)Input[0] << 8) | (uint16_t)Input[1]);
	shapeY = (int16_t)(((uint16_t)Input[2] << 8) | (uint16_t)Input[3]);
	Shape = Input + 4;

	ShapeOffset = (LongWord)((uintptr_t)Shape -
		(uintptr_t)ResourceBase);
	if (ShapeOffset >= ResourceSize) {
		ReleaseAResource(RezNum);
		return;
	}

	x = ((psp->WeaponX + shapeX) * (int)GunXScale) >> 20;
	y = ((psp->WeaponY + SCREENGUNY + shapeY) * (int)GunYScale) >> 16;

	x += ScreenXOffset;
	y += ScreenYOffset + 2;

	DrawWeaponShape(x, y, Shape,
		ResourceSize - ShapeOffset,
		(Fixed)GunXScale, (Fixed)GunYScale,
		(Boolean)Shadow);
